...
export const calculateArbitrageProfit = (prices) => {
  ...
  if (profitablePairs.length > 0) {
    // Знаходимо пару з найбільшим прибутком
    const mostProfitablePair = profitablePairs.reduce((maxPair, currentPair) =>
      currentPair.profitInPercentage > maxPair.profitInPercentage ? currentPair : maxPair
    );

    // Виводимо всі вигідні пари
    profitablePairs.forEach(pair => {
      console.log(`Вигідна пара ${pair.pairName}: ${pair.profitInPercentage}%`);
    });

    // Виводимо найбільш прибуткову пару
    console.log(`Найбільш прибуткова пара: ${mostProfitablePair.pairName} з прибутком ${mostProfitablePair.profitInPercentage}%`);
  } else {
